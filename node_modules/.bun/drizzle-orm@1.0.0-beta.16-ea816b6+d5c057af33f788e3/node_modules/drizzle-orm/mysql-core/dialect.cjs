	...
	*/
	...
	buildSelectQuery({ withList, fields, fieldsFlat, where, having, table, joins, orderBy, groupBy, limit, offset, lockingClause, distinct, setOperators, useIndex, forceIndex, ignoreIndex }) {
		...
		if (joins) for (const [index, joinMeta] of joins.entries()) {
			...
			if ((0, __entity_ts.is)(table$1, require_mysql_core_table.MySqlTable)) {
				const tableName = table$1[require_mysql_core_table.MySqlTable.Symbol.Name];
				const tableSchema = table$1[require_mysql_core_table.MySqlTable.Symbol.Schema];
				const origTableName = table$1[require_mysql_core_table.MySqlTable.Symbol.OriginalName];
				const alias = tableName === origTableName ? void 0 : joinMeta.alias;
				const useIndexSql$1 = this.buildIndex({
					indexes: joinMeta.useIndex,
					indexFor: "USE"
				});
				const forceIndexSql$1 = this.buildIndex({
					indexes: joinMeta.forceIndex,
					indexFor: "FORCE"
				});
				const ignoreIndexSql$1 = this.buildIndex({
					indexes: joinMeta.ignoreIndex,
					indexFor: "IGNORE"
				});
				joinsArray.push(__sql_sql_ts.sql`${__sql_sql_ts.sql.raw(joinMeta.joinType)} join${lateralSql} ${tableSchema ? __sql_sql_ts.sql`${__sql_sql_ts.sql.identifier(tableSchema)}.` : void 0}${__sql_sql_ts.sql.identifier(origTableName)}${useIndexSql$1}${forceIndexSql$1}${ignoreIndexSql$1}${alias && __sql_sql_ts.sql` ${__sql_sql_ts.sql.identifier(alias)}`}${onSql}`);
			} else if ((0, __entity_ts.is)(table$1, __sql_sql_ts.View)) {
				const viewName = table$1[__view_common_ts.ViewBaseConfig].name;
				const viewSchema = table$1[__view_common_ts.ViewBaseConfig].schema;
				const origViewName = table$1[__view_common_ts.ViewBaseConfig].originalName;
			...
			} else joinsArray.push(__sql_sql_ts.sql`${__sql_sql_ts.sql.raw(joinMeta.joinType)} join${lateralSql} ${table$1}${onSql}`);
	...
	_buildRelationalQuery({ fullSchema, schema, tableNamesMap, table, tableConfig, queryConfig: config, tableAlias, nestedQueryRelation, joinOn }) {
		...
		if (config === true) selection = Object.entries(tableConfig.columns).map(([key, value]) => ({
			...
			for (const { tsKey: selectedRelationTsKey, queryConfig: selectedRelationConfigValue, relation } of selectedRelations) {
				...
				const joinOn$1 = (0, __sql_expressions_index_ts.and)(...normalizedRelation.fields.map((field$1, i) => (0, __sql_expressions_index_ts.eq)((0, __alias_ts.aliasedTableColumn)(normalizedRelation.references[i], relationTableAlias), (0, __alias_ts.aliasedTableColumn)(field$1, tableAlias))));
		...
		if (nestedQueryRelation) {
			let field = __sql_sql_ts.sql`json_array(${__sql_sql_ts.sql.join(selection.map(({ field: field$1, tsKey, isJson }) => isJson ? __sql_sql_ts.sql`${__sql_sql_ts.sql.identifier(`${tableAlias}_${tsKey}`)}.${__sql_sql_ts.sql.identifier("data")}` : (0, __entity_ts.is)(field$1, __sql_sql_ts.SQL.Aliased) ? field$1.sql : field$1), __sql_sql_ts.sql`, `)})`;
	...
	_buildRelationalQueryWithoutLateralSubqueries({ fullSchema, schema, tableNamesMap, table, tableConfig, queryConfig: config, tableAlias, nestedQueryRelation, joinOn }) {
		...
		where = (0, __sql_expressions_index_ts.and)(joinOn, where);
		if (nestedQueryRelation) {
			let field = __sql_sql_ts.sql`json_array(${__sql_sql_ts.sql.join(selection.map(({ field: field$1 }) => (0, __entity_ts.is)(field$1, require_mysql_core_columns_common.MySqlColumn) ? __sql_sql_ts.sql.identifier(this.casing.getColumnCasing(field$1)) : (0, __entity_ts.is)(field$1, __sql_sql_ts.SQL.Aliased) ? field$1.sql : field$1), __sql_sql_ts.sql`, `)})`;
