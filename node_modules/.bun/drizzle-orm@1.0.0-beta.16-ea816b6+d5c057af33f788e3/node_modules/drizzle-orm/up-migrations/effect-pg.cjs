...
const upgradeFunctions = { 0: (migrationsSchema, migrationsTable, session, localMigrations) => {
	return effect.Effect.gen(function* () {
		...
		yield* session.execute(__sql_sql_ts.sql`ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS "name" text`);
		yield* session.execute(__sql_sql_ts.sql`ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS "applied_at" timestamp with time zone DEFAULT now()`);
		...
		for (const dbRow of dbRows) {
			...
			yield* session.execute(__sql_sql_ts.sql`UPDATE ${table} SET name = ${matched?.name ?? null}, applied_at = NULL WHERE id = ${dbRow.id}`);
		}
	});
} };
/**
