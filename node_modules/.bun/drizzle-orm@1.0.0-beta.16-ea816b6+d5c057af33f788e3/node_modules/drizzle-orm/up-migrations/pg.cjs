...
async function execute(session, sql$1) {
	const result = await session.execute(sql$1);
...
const upgradeFunctions = { 0: async (migrationsSchema, migrationsTable, session, localMigrations) => {
	...
	await execute(session, __sql_sql_ts.sql`ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS "name" text`);
	await execute(session, __sql_sql_ts.sql`ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS "applied_at" timestamp with time zone DEFAULT now()`);
	const dbRows = await execute(session, __sql_sql_ts.sql`SELECT id, hash, created_at FROM ${table} ORDER BY id ASC`);
	...
	for (const dbRow of dbRows) {
		...
		await execute(session, __sql_sql_ts.sql`UPDATE ${table} SET name = ${matched?.name ?? null}, applied_at = NULL WHERE id = ${dbRow.id}`);
	}
} };
/**
...
async function upgradeIfNeeded(migrationsSchema, migrationsTable, session, localMigrations) {
	if ((await execute(session, __sql_sql_ts.sql`SELECT 1 FROM information_schema.tables
			WHERE table_schema = ${migrationsSchema}
			AND table_name = ${migrationsTable}`)).length === 0) return { newDb: true };
	let version = getVersion((await execute(session, __sql_sql_ts.sql`SELECT
			n.nspname AS "schema",
