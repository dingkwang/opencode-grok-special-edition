...
const upgradeFunctions = { 0: async (migrationsSchema, migrationsTable, session, localMigrations) => {
	...
	await session.execute(__sql_sql_ts.sql`ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS "name" text`);
	await session.execute(__sql_sql_ts.sql`ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS "applied_at" timestamp with time zone DEFAULT now()`);
...
async function upgradeIfNeeded(migrationsSchema, migrationsTable, session, localMigrations) {
	if (!(await session.all(__sql_sql_ts.sql`SELECT EXISTS (
			SELECT FROM information_schema.tables
			WHERE table_schema = ${migrationsSchema}
			AND table_name = ${migrationsTable}
		)`))[0]?.exists) return { newDb: true };
	let version = getVersion((await session.all(__sql_sql_ts.sql`SELECT
			n.nspname AS "schema",
