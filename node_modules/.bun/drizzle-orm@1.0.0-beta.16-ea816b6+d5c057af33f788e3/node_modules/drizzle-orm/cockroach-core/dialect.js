...
const upgradeFunctions = { 0: async (migrationsSchema, migrationsTable, session, localMigrations) => {
	...
	await session.execute(sql`ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS "applied_at" timestamp with time zone DEFAULT now()`);
	...
	for (const dbRow of dbRows) {
		...
		await session.execute(sql`UPDATE ${table} SET name = ${matched?.name ?? null}, applied_at = NULL WHERE id = ${dbRow.id}`);
	}
} };
/**
...
async function upgradeIfNeeded(migrationsSchema, migrationsTable, session, localMigrations) {
	if (!(await session.all(sql`SELECT EXISTS (
