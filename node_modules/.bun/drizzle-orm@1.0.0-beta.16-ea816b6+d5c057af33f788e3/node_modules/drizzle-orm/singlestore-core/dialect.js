...
var SingleStoreDialect = class {
	...
	*/
	...
	buildSelectQuery({ withList, fields, fieldsFlat, where, having, table, joins, orderBy, groupBy, limit, offset, lockingClause, distinct, setOperators }) {
		...
		for (const f of fieldsList) if (is(f.field, Column) && getTableName(f.field.table) !== (is(table, Subquery) ? table._.alias : is(table, SQL) ? void 0 : getTableName(table)) && !((table$1) => joins?.some(({ alias }) => alias === (table$1[Table.Symbol.IsAlias] ? getTableName(table$1) : table$1[Table.Symbol.BaseName])))(f.field.table)) {
		...
		if (joins) for (const [index, joinMeta] of joins.entries()) {
			...
			if (is(table$1, SingleStoreTable)) {
				const tableName = table$1[SingleStoreTable.Symbol.Name];
				const tableSchema = table$1[SingleStoreTable.Symbol.Schema];
				const origTableName = table$1[SingleStoreTable.Symbol.OriginalName];
			...
			} else if (is(table$1, View)) {
				const viewName = table$1[ViewBaseConfig].name;
				const viewSchema = table$1[ViewBaseConfig].schema;
				const origViewName = table$1[ViewBaseConfig].originalName;
			...
			} else joinsArray.push(sql`${sql.raw(joinMeta.joinType)} join${lateralSql} ${table$1}${onSql}`);
	...
	_buildRelationalQuery({ fullSchema, schema, tableNamesMap, table, tableConfig, queryConfig: config, tableAlias, nestedQueryRelation, joinOn }) {
		...
		if (nestedQueryRelation) {
			let field = sql`JSON_TO_ARRAY(${sql.join(selection.map(({ field: field$1, tsKey, isJson }) => isJson ? sql`${sql.identifier(`${tableAlias}_${tsKey}`)}.${sql.identifier("data")}` : is(field$1, SQL.Aliased) ? field$1.sql : field$1), sql`, `)})`;
	...
	buildRelationalQuery({ schema, table, tableConfig, queryConfig: config, relationWhere, mode, errorPath, depth, isNestedMany, throughJoin }) {
		...
		const joins = params ? (() => {
			...
			return sql.join(withEntries.map(([k, join]) => {
				...
				selectionArr.push(isSingle$1 ? sql`${sql.identifier(k)}.${sql.identifier("r")} as ${sql.identifier(k)}` : sql`coalesce(${sql.identifier(k)}.${sql.identifier("r")}, json_build_array()) as ${sql.identifier(k)}`);
				...
				return sql` left join lateral(select ${sql`${isSingle$1 ? sql`json_build_object(${jsonColumns})` : sql`json_agg(json_build_object(${jsonColumns})${innerQuery.order ? sql` ORDER BY ${sql.identifier(`$drizzle_order_row_number`)}` : void 0})`} as ${sql.identifier("r")}`} from (${innerQuery.sql}) as ${sql.identifier("t")}) as ${sql.identifier(k)} on true`;
			}));
		})() : void 0;
