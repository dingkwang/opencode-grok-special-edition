...
async function migrate(migrations, session, config) {
	...
	const { newDb } = await (0, __up_migrations_pg_ts.upgradeIfNeeded)(migrationsSchema, migrationsTable, session, migrations);
	if (newDb) {
	...
	if (typeof config === "object" && config.init) {
		...
		if (!migration) return;
		await session.execute(__sql_sql_ts.sql`insert into ${__sql_sql_ts.sql.identifier(migrationsSchema)}.${__sql_sql_ts.sql.identifier(migrationsTable)} ("hash", "created_at", "name") values(${migration.hash}, ${migration.folderMillis}, ${migration.name ?? null})`);
		return;
	}
