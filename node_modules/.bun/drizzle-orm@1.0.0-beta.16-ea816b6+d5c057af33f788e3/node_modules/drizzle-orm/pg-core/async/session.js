...
async function migrate(migrations, session, config) {
	...
	if (typeof config === "object" && config.init) {
		...
		if (!migration) return;
		await session.execute(sql`insert into ${sql.identifier(migrationsSchema)}.${sql.identifier(migrationsTable)} ("hash", "created_at", "name") values(${migration.hash}, ${migration.folderMillis}, ${migration.name ?? null})`);
		return;
	}
