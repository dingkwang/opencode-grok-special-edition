    ...
    async success(ctx, properties, successOpts) {
      return await input.success({
        async subject(type, id, unvalidated, subjectOpts) {
          ...
          let properties2 = unvalidated;
          const validator = input.subjects?.[type];
          if (validator) {
            const validated = await validator["~standard"].validate(properties2);
            if (validated.issues) {
              throw new Error(validated.issues.map((i) => i.message).join(`
`));
            }
            properties2 = validated.value;
          }
  ...
  async function generateTokens(ctx, value, opts) {
    ...
    const expiry = Math.floor((value.timeUsed ?? Date.now()) / 1000) + value.ttl.access;
    return {
      access: await new SignJWT({
        ...
        iss: issuer2(ctx),
        sub: value.subject
      }).setExpirationTime(expiry).setProtectedHeader(await signingKey().then((k) => ({
        alg: k.alg,
        kid: k.id,
        typ: "JWT"
  ...
  app.get("/userinfo", async (c) => {
    ...
    if (schema) {
      const result2 = await schema["~standard"].validate(validated);
      if (result2.issues) {
        return c.json({
          error: "invalid_token",
