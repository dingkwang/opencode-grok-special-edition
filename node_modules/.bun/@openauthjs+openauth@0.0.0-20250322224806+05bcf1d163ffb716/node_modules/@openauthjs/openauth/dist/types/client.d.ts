...
export interface VerifyResult<T extends SubjectSchema> {
    ...
    subject: {
        [type in keyof T]: {
            id: string;
            type: type;
            properties: v1.InferOutput<T[type]> extends unknown ? Record<string, any> : v1.InferOutput<T[type]>;
        };
    }[keyof T];
}
/**
...
export interface Client<S extends SubjectSchema> {
     ...
     * ```
     */
    verify(token: string, options?: VerifyOptions): Promise<VerifyResult<S> | VerifyError>;
    /**
     ...
     * ```
     */
    decode<T extends SubjectSchema>(token: string, subjects: T): DecodeSuccess<T> | DecodeError;
}
/**
...
export declare function createClient<S extends SubjectSchema>(input: ClientInput<S>): Client<S>;
export {};
